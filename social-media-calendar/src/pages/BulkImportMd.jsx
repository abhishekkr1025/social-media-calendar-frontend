...
/**
    Parses a file's YAML frontmatter (the block between the leading --- lines)
    into a plain key/value object. Handles:
      - simple scalars:      category: World
      - quoted scalars:      title: "Some Title"
      - inline flow lists:   tags: [a, b, c]
      - block lists:         tags:
                                - a
                                - b
    This is intentionally lightweight — just enough for the fields the
    importer cares about (category, slug, tags), not a full YAML parser.
*/
function parseFrontmatter(rawText) {
    ...
    while (i < lines.length) {
        ...
        if (rawValue === "") {
            // Possible block-style list under this key
            const listItems = [];
            let j = i + 1;
    ...
    async function handleFileChange(e) {
        ...
        const newRows = await Promise.all(mdFiles.map(async (file) => {
            ...
            try {
                ...
                if (fm.category) detectedCategory = fm.category;

                if (fm.slug) {
                    detectedSlug = normalizeSlug(fm.slug);
                }
